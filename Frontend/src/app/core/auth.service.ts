...
//   return this.http.post<any>(`${API_CONFIG.AUTHORITY_BY_EMP}/login`, payload).pipe(
//     tap({
//       next: (res) => {
//         // Save session details to localStorage
//         const userObj = {
//           empCode: res.empNo || empNo,
//           role: res.role || 'User'
//         };
//         localStorage.setItem('currentUser', JSON.stringify(userObj));
        
//         // Update reactive state signals/variables in auth service
...
//       error: (err) => {
//         if (err.status === 401) {
//           this.resolveError.set('Invalid Employee Code or Password.');
//         } else if (err.status === 404) {
//           this.resolveError.set(`Employee Code "${empNo}" not found.`);
//         } else {
//           this.resolveError.set('Server error occurred during login.');
//         }
//       }
//     })
//   );
