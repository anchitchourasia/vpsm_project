...
export class AuthService {
  ...
  // ── RESET / UPDATE PASSWORD METHOD ──────────────────────────────
  resetPassword(empNo: string, newPassword: string): Observable<any> {
    this.clearError();
    ...
    return this.http.put<any>(API_CONFIG.AUTHORITY_UPDATE, payload, { headers: this.HEADERS }).pipe(
      timeout(12_000),
      catchError(err => {
        if (err?.status === 404) {
          this._error.set(`Employee Code "${empNo}" is not registered.`);
        } else if (err?.status === 0) {
          this._error.set('Cannot reach server. Check network.');
        } else {
          this._error.set(err?.error?.message || 'Password update failed.');
        }
        return throwError(() => err);
      })
    );
