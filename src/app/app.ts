...
export class App implements OnInit {
  ...
  openPassEntry(): void {
    window.location.href = "/pass-entry";
  }
  getCvpsVerifierUrl(): string {
    const empCode = String(this.auth.empCode() ?? "").trim();

    return `http://192.168.9.130:9092/cvps-ui/?empCode=${encodeURIComponent(empCode)}`;
  }
