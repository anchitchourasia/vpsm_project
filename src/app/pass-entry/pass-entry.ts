 ...
 */
  ...
  /**
   * Downloads a file selected locally before Save/Submit.
   * This is required if HTML calls:
   * (click)="downloadSelectedFile(doc)"
   */
  downloadSelectedFile(doc: PassDocument): void {
    if (!doc.file) {
      return;
    }
  ...
  /**
   * Opens a locally selected file in a separate tab.
   * Keep it only if your HTML still calls viewDocument(doc.file).
   */
  viewDocument(file: File | null): void {
    if (!file) {
      return;
    }
  ...
  /**
   * Downloads the document that is already stored on the server.
   *
   * This uses API_CONFIG. Add PASS_DOCUMENT_DOWNLOAD in api.config.ts
